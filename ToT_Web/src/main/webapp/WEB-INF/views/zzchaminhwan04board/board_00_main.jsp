...
<table>
	...
	<tbody>	
		...
		<c:forEach items="${boardVO.list }" var="vo">
		<tr>
			<td>${vo.board_sn }</td>
			<td class='left'><a onclick='go_detail(${vo.board_sn})'>${vo.board_title }</a></td>
			<td>${vo.member_id }</td>
			<td>${vo.board_create_date }</td>
			<td>${vo.board_readcnt }</td>
			<td>${empty vo.picture_filepath ? '' : '<img src="imgs/favicon.io" class="file-img" />'}</td>			
		</tr>
		</c:forEach>
	</tbody>
</table>
