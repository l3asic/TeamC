...
public class IotMainActivity extends AppCompatActivity {
    ...
    LinearLayout lin_iot_back, lin_iot_onoff, lin_iot_detail;
    ...
    double lac = 0 ;
    double loc = 0 ;
    //35.15362845906736, 126.88799571611783
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lin_iot_onoff.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (on_off == 1){
                    on_off = 0;
                    iotOnOffSet();
                    Toast.makeText(IotMainActivity.this, "세이프 모드 OFF", Toast.LENGTH_SHORT).show();
                    imgv_iot_onoff.setBackgroundTintList(ColorStateList.valueOf(Color.parseColor("#999999")));
                    ...
                    on_off = 1;
                    iotOnOffSet();
                    Toast.makeText(IotMainActivity.this, "세이프 모드 ON", Toast.LENGTH_SHORT).show();
                    imgv_iot_onoff.setBackgroundTintList(ColorStateList.valueOf(Color.parseColor("#2BA0DA")));
        ...
        // 최근 위치 확인 버튼 클릭시  (지도로 이동)
        lin_iot_detail.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                // 위치 정보가 있다면
                if(lac != 0 && loc != 0 ){
                    Intent intent = new Intent(IotMainActivity.this, KakaoMapActivity.class);

                    intent.putExtra("dto",dto);
                    startActivity(intent);
                }else{
                    Toast.makeText(IotMainActivity.this, "저장된 위치 정보가 없습니다.", Toast.LENGTH_SHORT).show();
                }
            ...
            if(on_off == 1){
                switch (action){
                    ...
                    case BluetoothDevice.ACTION_ACL_DISCONNECTED:   //블루투스 기기 끊어짐
                        ...
                        // 시간, 위경도 디비 저장
                        dto = new IotDTO(Logined.member_id, lac, loc, get_day, get_time,1);
...
//            finish();  // App 종료
    ...
    private void insertIot() {
        ...
        commonAsk = new CommonAsk("android/iot/insertIot");
        commonAsk.params.add(new CommonAskParam("member_id", Logined.member_id));
        InputStream in = CommonMethod.excuteAsk(commonAsk);
        on_off = 0;
