...
var join = {
	...
	// 인지를 구분(판단)하여 유효성 검사 진행하고자 함.
		...
		else if (tag == 'nick')	 return this.nick_status ( data );	 	 	
	}
	...
	, nick_status : function ( nick ) { // nick 의 상태 확인
	
		/* 입력값이 없을 경우 */
		if(nick == '')	return this.common.empty;
		/* 입력값에 공백이 있을 경우 */
		else if(nick.match(space))	return this.common.space;
		/* 10자 이상 입력 여부 확인*/
		else if(nick.length > 10 )	return this.common.max;
	}
	, nick: { // nick의 기준 설정
	...
	, nick_usable : function ( usable ) {
		if (usable)	return this.nick.usable;
		else 		return this.nick.unUsable;
	}
